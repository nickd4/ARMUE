...
{
    _timer_t *timer = (_timer_t *)calloc(1, sizeof(_timer_t));
    if(timer == NULL){
        goto timer_error;
    }
...
list_t *find_timer(int exception_num, list_t *timer_list)
{
    ...
    for_each_list_node(cur, timer_list){
        cur_timer = (_timer_t *)cur->data.pdata;
...
int add_timer(_timer_t *timer, list_t *timer_list, bool_t ignore_duplicate)
{
    ...
    if(cur != NULL){
        ...
        if(ignore_duplicate){
            ...
            timer_found = (_timer_t *)cur->data.pdata;
...
void check_timer(cpu_t *cpu)
{
    ...
    for_each_list_node_safe(cur_node, next_node, timer_list){
        timer = (_timer_t *)cur_node->data.pdata;
